...
ビルド済みの`LightWatch.app`を`/Applications`へ配置して起動します。`ログイン時に常駐`を有効にすると、ユーザーのLaunchAgentへ登録され、ログイン時に起動します。監視プロセスが異常終了した場合は自動的に再起動します。macOSのカメラ権限はログイン中のユーザーに付与されるため、root権限や`LaunchDaemon`は使用しません。
